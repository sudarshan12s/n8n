...
- Output Dimensions uses a model-capability lookup. It currently has a documented fallback for `cohere.embed-v4.0`: 256, 512, 1,024, and 1,536. Default leaves the parameter unset, so OCI selects its native default of 1,536. The option is hidden for models without verified output-dimension metadata, and execution rejects dimensions supplied for unsupported models. Replace the fallback with OCI catalog metadata when that metadata is reliably available.
