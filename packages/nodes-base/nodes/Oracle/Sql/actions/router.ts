...
export async function router(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	const continueOnFail = this.continueOnFail() || node.onError === 'continueErrorOutput';
	...
	try {
		...
		const errorItems =
			items.length > 0
				? items.map((_item, index) => prepareErrorItem(items, error as Error, index))
				: [prepareErrorItem(items, error as Error, 0)];
